# ============================================================
# SOXL Strategy v19 (SOXS_ma修正 + SC_lock追加) - GitHub Actions + LINE通知
# ------------------------------------------------------------
# 変更点 (V18 -> V19)：
...
SC_LOCK_DAYS    = 3       # ★ V19新規: SC SL後ロック日数
...
PARAMS = {
    ...
    "SNIPER":      {"tp": 0.16, "sl": -0.10, "hold": 20},
}
...
def push_state_to_github():
    ...
    if diff.returncode == 0:
        ...
        return
    subprocess.run(["git", "commit", "-m", "Update state.json v19"], check=True)
...
def fetch_data() -> pd.DataFrame:
    ...
    df["sig_soxs_pre_dc"] = ((df["soxl_ma_20"] > df["soxl_ma_50"]) & (df["close"] < df["soxl_ma_20"]) &
                              (df["soxl_dist"] < 0.010) & (df["vix_ret5"] > 0.10))
    df["sig_soxs_bb"]     = (df["close"] >= df["bb40_upper"])
    df["sig_soxs_ma"]     = ((df["ma75_dev"] >= SOXS_MA_MIN_DEV) &
...
def build_message(today, state_after, action, action_reason,
                  soxs_ma_locked, lock_days_passed,
                  sc_locked, sc_lock_days_passed,
                  df, today_date,
    ...
    lines = []
    lines.append(f"【SOXL戦略 v19 日次レポート {date_str}】")
    lines.append("")
    ...
    if bool(today['sig_base_alert']):
        ...
        sc_lock_note = " 🔒ロック中" if sc_locked else ""
        lines.append(f" └ SC ✅{sc_lock_note}")
    ...
    # ★ V19新規: SCロック表示
    if sc_locked:
        lines.append(f"SC      : 🔒 SCロック中 (解除まであと{SC_LOCK_DAYS + 1 - sc_lock_days_passed}日)")
    lines.append("")
...
def main():
    ...
    if (not state_after.get("sniper_pos") and
        not state_after.get("sniper_pending_entry") and
        not state_after.get("sniper_pending_exit")):
    ...
    # ★ V19新規: SC SLロック判定
    sc_lock_days_passed = 0
    sc_locked           = False
    if state_before.get("last_sc_sl_date"):
        sc_lock_days_passed = count_hold_days(df, state_before["last_sc_sl_date"], today_date) - 1
        if sc_lock_days_passed <= SC_LOCK_DAYS:
            sc_locked = True
        else:
            state_after["last_sc_sl_date"] = None
    ...
    if state_after["position"] in ["SOXL", "SOXS"] and state_after.get("entry_price"):
        ...
        if t_l <= sl_px:
            ...
            # ★ V19新規: SC SLロック記録
            if pos == "SOXL" and sig_t in ["sc", "SC"]:
                state_after["last_sc_sl_date"] = str(today_date.date())
        elif t_h >= tp_px:
            action        = f"{pos}売却(TP)"
    ...
    # ★ V19新規: SCロック時はSCをv9_sigから除外
    if sc_locked and bool(today["sig_sc"]):
        v9_sig_no_sc = (bool(today["sig_base_alert"]) or
                        bool(today["sig_base_normal"]) or
                        bool(today["sig_bb"]) or
                        bool(today["sig_or4"]))
        v9_sig = v9_sig_no_sc

    if state_after["position"] == "CASH":
        target = None
        if v9_sig:
            sig_n  = get_v9_type(today)
            # SCがロックされている場合は別シグナルタイプにフォールバック
            if sig_n == "sc" and sc_locked:
                sig_n = "base_normal" if bool(today["sig_base_normal"]) else "or4"
            target = ("SOXL", sig_n)
        elif dip_sig:
    ...
    msg = build_message(
        today, state_after, action, action_reason,
        soxs_ma_locked, lock_days_passed,
        sc_locked, sc_lock_days_passed,
        df, today_date,
        sniper_action, sniper_reason
    )
    print(msg)
    send_line(msg)
